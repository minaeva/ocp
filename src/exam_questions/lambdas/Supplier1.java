...
then type inference of lambda expression is based on following from the JLS:

If T is a wildcard-parameterized functional interface type
and the lambda expression is implicitly typed,
then the ground target type is the non-wildcard parameterization of T.

Based on above JLS statement, ground target type of above lambda expression will be:
...
There is no issue with lambda expressions (method invocation)
...
There is no issue with the lambda expression of 2nd statement (method invocation)
